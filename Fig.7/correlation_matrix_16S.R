...
library("corrplot")
####assinging data file##########
bdiv_all <- ("correlation_rawdata.csv")
...
###############
bdiv_micro <- ("correlation_rawdata_micro.csv")
...
# Insignificant correlations are leaved blank
corrplot(res2$r, type="upper", order="original",
         method = c("circle"), tl.col = "black", tl.srt=60,  tl.cex = 1.6, number.font = 6,
         col=colorRampPalette(c("darkorange","white","darkgreen"))(100),cl.lim=c(0,1), number.cex = 1,
         p.mat = res2$P, sig.level = 0.05, insig = "blank")
###############################################################

bdiv_macro <- ("correlation_rawdata_macro.csv")
